 ...
 *
 * Configuration is user-owned, exactly like the official deepseek provider:
 * the plugin ships only the provider row; endpoint selection, API key and
 * tool arguments are supplied by the user in their own profile patch or
 * via environment variables. Nothing is built in.
 *
 ...
 * (`web-search-zhipu`) into the settings service for hot edits. Each search
 * call is audited into the initiating agent's session
 * (`web/zhipu-search-mcp-request`: endpoint, tool, merged arguments).
 *
...
/** Default Zhipu MCP endpoint: the web_search_prime service. */
const DEFAULT_MCP_URL = "https://open.bigmodel.cn/api/mcp/web_search_prime/mcp";
/** MCP tool name for the web_search_prime service. */
const DEFAULT_TOOL = "web_search_prime";
...
function resolveOptions(section) {
	return {
		...
		tool: section.tool ?? DEFAULT_TOOL,
...
class ZhipuSearchProvider {
	...
	available() {
		...
		return resolveApiKey(options) !== undefined && URL.canParse(options.mcpURL);
	}
	...
	async #mcpSearch(request, apiKey, options, signal) {
		...
		// Audit the merged arguments (esp. toolArguments overrides) into the
		// initiating agent's session — official deepseek provider pattern.
		this.recordRequest?.({
		...
		const attempt = async (sessionId) => {
			...
			const { payload } = await this.#rpc({
				...
				method: "tools/call",
				params: { name: options.tool, arguments: args },
		...
		if (session !== undefined && (session.url !== options.mcpURL || session.apiKey !== apiKey)) {
			session = undefined;
		}
		let payload;
		...
		if (session === undefined) {
			...
			this.#session = { url: options.mcpURL, apiKey, id };
...
const Config = z.object({
	...
	/** Zhipu MCP endpoint; defaults to the web_search_prime service. */
	mcpURL: z.string().default(DEFAULT_MCP_URL),
	/** MCP tool name called on the endpoint. */
...
function apply(ctx, config) {
	...
	ctx.web.registerSearchProvider(new ZhipuSearchProvider(
		() => resolveOptions(current()),
		(request) => {
			ctx.get("agents")?.currentInitiator()?.session.append("web/zhipu-search-mcp-request", request);
		},
	));
}
